# класс объектов-итераторов для указания текущего элемента
# в последовательности
class RangeIterator:
    # при создании создаёт переменные для хранения текущего состояния
    ...
    def __next__(self):
        self.x += 1
        # если последовательность итератора не бесконечна - должно быть условие
        # для окончания цикла
        if self.x > self.size:
...
class RangeIterable:
    ...
    # в начале цикла вызывается метод __iter__ для получения итератора
